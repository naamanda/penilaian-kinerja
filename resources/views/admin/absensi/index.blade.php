...
<div class="pt-2 px-6 pb-6">
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        <form method="GET" action="/absensi" class="flex flex-wrap items-center gap-3">
            ...
            <button type="submit" class="bg-[#1e3f7c] hover:bg-blue-900 text-white text-sm font-semibold h-10 px-5 rounded-xl shadow-sm transition-all flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2l-7 7v5l-4 2v-7L3 6V4z"/></svg>
                Filter
            </button>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($data as $a)
                    {{-- Hover efek baris diganti ke warna biru lembut (hover:bg-blue-50/60) --}}
                    <tr class="hover:bg-blue-50/60 transition-colors duration-200">
                        ...
                        <td class="px-6 py-3.5 text-center whitespace-nowrap">
                            <div class="flex justify-center items-center gap-4">
                                {{-- Tombol "Lihat Detail" Gabungan Teks + Ikon Mata --}}
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-10 text-center text-gray-400 font-medium">Belum ada data absensi</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="mt-4 flex justify-end">
        <div class="flex items-center gap-3">
            <span class="text-sm text-gray-500">Showing {{ $data->firstItem() }} to {{ $data->lastItem() }} of {{ $data->total() }} results</span>
            <div class="flex items-center gap-1">
                @for($i = 1; $i <= $data->lastPage(); $i++)
                    <a href="{{ $data->url($i) }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-sm font-medium transition-all {{ $data->currentPage() == $i ? 'bg-[#1e3f7c] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-blue-50/60' }}">
                        {{ $i }}
                    </a>
                @endfor
            </div>
                ...
                <div class="bg-white px-6 pt-6 pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2"><p class="text-sm text-gray-500 italic">Apakah Anda yakin ingin menghapus data absensi <span id="modal-absensi" class="font-bold text-gray-800 not-italic"></span>? Tindakan ini tidak bisa dibatalkan.</p></div>
                        </div>
                    </div>
                </div>
